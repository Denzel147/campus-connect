...
const getAllUsers = async (req, res, next) => {
  try {
    ...
    if (status && status !== 'all') {
      whereClause += `WHERE account_status = $${paramCount}`;
      queryParams.push(status);
      paramCount++;
    }

    if (search) {
      const searchClause = `${whereClause ? 'AND' : 'WHERE'} (full_name ILIKE $${paramCount} OR email ILIKE $${paramCount + 1})`;
    ...
    const totalCount = rows.length > 0 ? Number.parseInt(rows[0].total_count) : 0;
...
const getAllItems = async (req, res, next) => {
  try {
    ...
    if (status && status !== 'all') {
      whereClause += `WHERE i.availability_status = $${paramCount}`;
      queryParams.push(status);
      paramCount++;
    }

    if (category && category !== 'all') {
      const categoryClause = `${whereClause ? 'AND' : 'WHERE'} i.category = $${paramCount}`;
    ...
    if (search) {
      const searchClause = `${whereClause ? 'AND' : 'WHERE'} (i.item_name ILIKE $${paramCount} OR i.description ILIKE $${paramCount + 1})`;
      whereClause += searchClause;
      queryParams.push(`%${search}%`, `%${search}%`);
      paramCount += 2;
    }
    ...
    const query = `
      ...
      ${whereClause}
      GROUP BY i.item_id, u.user_id, c.category_id
...
const getAllTransactions = async (req, res, next) => {
  try {
    ...
    if (status && status !== 'all') {
      whereClause += `WHERE t.transaction_status = $${paramCount}`;
      queryParams.push(status);
      paramCount++;
    }

    if (search) {
      const searchClause = `${whereClause ? 'AND' : 'WHERE'} (i.item_name ILIKE $${paramCount} OR lender.full_name ILIKE $${paramCount + 1} OR borrower.full_name ILIKE $${paramCount + 2})`;
      whereClause += searchClause;
  ...
  try {
    ...
    const query = `
      SELECT 
        ...
        t.due_date as expected_return_date,
        t.return_date as actual_return_date,
        t.date_created as request_date,
        u.user_id,
        u.full_name,
        u.email,
        u.phone_number,
...
async function getUserStatistics() {
  const query = `
    SELECT 
      ...
      COUNT(CASE WHEN verification_status != 'verified' OR verification_status IS NULL THEN 1 END) as unverified
    FROM users
  `;
  const { rows } = await db.query(query);
  return rows[0];
